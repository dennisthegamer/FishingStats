...
import de.dennisthegamer.fishingstats.mixin.ItemEntityAccessor;
...
public class FishingTracker {
    ...
    private void onCast(FishingHook hook, LocalPlayer player, Level level, long now) {
        hookId = hook.getId();
        wasBiting = false;
        castTimeMs = now;
        biteTimeMs = -1;
    ...
    private void watchForLoot(Level level, long now) {
        ...
        if (candidateItemId == -1) {
            AABB box = new AABB(
                    ...
                    expectedPos.x + LOOT_RADIUS, expectedPos.y + LOOT_RADIUS, expectedPos.z + LOOT_RADIUS);
            for (ItemEntity item : level.getEntitiesOfClass(ItemEntity.class, box, item -> true)) {
                if (((ItemEntityAccessor) (Object) item).fishingStats$getAge() <= MAX_ITEM_AGE_TICKS) {
    ...
    private static String bookDetail(ItemStack stack) {
        ...
        stored.entrySet().forEach(entry -> {
            ...
            entry.getKey().unwrapKey().ifPresent(key -> sb.append(Platforms.get().keyId(key)));
    ...
    private static RodEnchantments readRodEnchantments(LocalPlayer player) {
        ...
        try {
            ItemEnchantments enchants = EnchantmentHelper.getEnchantmentsForCrafting(rod);
            int luck = 0, lure = 0, unbreaking = 0, mending = 0;
            for (var entry : enchants.entrySet()) {
                var key = entry.getKey();
                if (key.is(Enchantments.LUCK_OF_THE_SEA)) luck = entry.getIntValue();
                else if (key.is(Enchantments.LURE)) lure = entry.getIntValue();
                else if (key.is(Enchantments.UNBREAKING)) unbreaking = entry.getIntValue();
                else if (key.is(Enchantments.MENDING)) mending = entry.getIntValue();
            }
            String name = rod.has(DataComponents.CUSTOM_NAME)
                    ? rod.getHoverName().getString() : null;
